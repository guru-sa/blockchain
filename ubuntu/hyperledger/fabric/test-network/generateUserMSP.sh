...
set -x
fabric-ca-client enroll -u https://user1:user1pw@localhost:${port} --caname ca-${org} -M "${PWD}/organizations/peerOrganizations/${domain}/users/User1@${domain}/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/${org}/tls-cert.pem"
{ set +x; } 2>/dev/null
