...
const (
	...
	timestamp = `"2021-09-01T08:54:56Z"`
...
func TestInvoke_NG1(t *testing.T) {
	stub := shim.NewMockStub("cartransfer", new(chaincode.CarTransferCC))
	if assert.NotNil(t, stub) && assert.Condition(t, responseOK(stub.MockInit(util.GenerateUUID(), nil))) {
...
func TestUpdateCar_OK1(t *testing.T) {
	...
	if assert.NotNil(t, stub) && assert.Condition(t, responseOK(stub.MockInit(util.GenerateUUID(), nil))) {
		...
		res = stub.MockInvoke(util.GenerateUUID(), getBytes("GetCar", one))
		if assert.Condition(t, responseFail(res)) {
...
// OK1: transfer from Alice to Bob
